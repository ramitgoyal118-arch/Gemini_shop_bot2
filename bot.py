...
# =========================================================
BOT_TOKEN = "8972099567:AAFEkZg_NOXlfAg0DWnwJePCFQ19nEcyXB4"
OWNER_ID = 7737039539
...
def unique_order_id():
    import secrets
    return "GM" + secrets.token_hex(4).upper()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        product_text(update.effective_user.id), reply_markup=main_keyboard()
    )
...
async def warranty_proof(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if OWNER_ID:
        await context.bot.send_message(
            ...
            f"Link: {claim['link']}\n"
            f"Proof: {proof[:1500]}\n\n"
            f"/warranty_approve {claim_id}\n"
            f"/warranty_reject {claim_id} REASON\n"
            f"/warranty_resolve {claim_id} REPLACEMENT_LINK"
        )
...
async def warranty_reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id) or len(context.args) < 1:
        return
    cid = int(context.args[0])
